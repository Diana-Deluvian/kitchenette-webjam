...
const Search = () => {
  ...
  const categoryClassList = [
    `w-max text-center px-4 py-2 cursor-pointer
  hover:bg-primary hover:text-white duration-300 ease-in`,
  ];

  return (
    <div
      className='flex flex-col w-xl items-center min-w-24 mt-8 border-t-2 border-primary'
      id='search-container'
    >
      <div className='flex flex-wrap justify-center '>
        ...
        ${searchCategory === category ? 'bg-primary text-white' : null}`}
          >
            {category}
          </span>
        ))}
