...
const RecipeForm = (props) => {
  ...
  const [instructions, setInstructions] = useState(
    props.recipe ? props.recipe.instructions : ['']
  );
  ...
  const ingredientsList = ingredients.map((ingredient, index) => (
    ...
    >
      ...
      <button
        ...
        className='text-primary absolute right-4 top-2'
      >
        X
      </button>
    </div>
  ...
  const instructionsList = instructions.map((instruction, index) => (
    ...
    >
      ...
      <textarea
        ...
        value={instruction}
        rows='2'
        className='outline-none pl-2 pr-4 bg-gray-300 md:w-80'
      ></textarea>
      <button
        data-instructionindex={index}
  ...
  const handleImgChange = () => {
    if (img.current.files[0].name.length > 15)
      setImgName(img.current.files[0].name.substring(0, 15) + '...');
    else setImgName(img.current.files[0].name);
  };
  const categoriesList = categories.map((category) => (
    <li className='cursor-pointer py-1' onClick={handleCategoryChange}>
      {category}
    </li>
  ));
  ...
  if (!isAuth) {
    return (
      <div className='flex flex-col'>
        ...
        <Link
          className='bg-secondary p-4 text-white rounded mt-4 w-max text-center'
          to='/'
        >
  ...
  return (
    <form
      className='w-screen-lg flex flex-col items-center text-center relative'
      onSubmit={handleOnSubmit}
    >
      <h1 className='text-4xl my-6 text-primary text-center font-emilysCandy'>
        New recipe!{' '}
      </h1>
      ...
      <div className='flex flex-col md:flex-row w-full justify-evenly p-3'>
        ...
        <div className='flex flex-col items-center'>
          ...
          <button
            ...
            className='relative py-1 px-3 border-b-2 border-primary w-32'
          >
            {' '}
            {recipe.category}{' '}
          </button>
      ...
      <div className='flex flex-col justify-around items-center md:items-start md:flex-row mt-8'>
        <div className='md:mr-12'>
          <h2 className='mb-4 text-secondary font-emilysCandy text-2xl'>
            Ingredients!
          </h2>
          {ingredientsList}
          <button
            className='bg-secondary text-white p-2 px-8 rounded mt-2'
        ...
        <div className=''>
          <h2 className='mb-6 text-secondary font-emilysCandy text-2xl'>
            Instructions!
          </h2>
          {instructionsList}
          <button
            className='bg-secondary text-white p-2 px-8 rounded'
      ...
      <div className='flex flex-col mt-4 items-center content-center mb-4'>
        <label className='bg-secondary px-6 py-2 rounded cursor-pointer text-white'>
          Select Image:
          <input
            type='file'
            ref={img}
            onChange={handleImgChange}
            className='p-2 flex items-center content-center hidden'
          />
        </label>
        {imgName.length === 0 ? (
          <p className='border-b-2 text-gray-500 border-primary w-48 mt-2'>
            Your image here
          </p>
        ) : (
      ...
      <input
        type='submit'
        value={props.isCRUDLoading ? 'Submitting...' : 'Submit'}
        className='w-48 p-2 rounded bg-primary text-white mb-12 cursor-pointer'
      />
